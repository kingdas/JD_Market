<template>
  <div class="page">
    <div class='search-header'>
      ...
      <div class='search' @click="searchShow.show = true">请输入宝贝名称</div>
    </div>
    <div class='goods-main'>
      <div ref="scroll-classify" class='classify-wrap'>
        <div>
          <div ref="item" :class="{'classify-item':true, active:item.active}" v-for="(item,index) in classifys"
               :key="index" @click="replacePage('/classify/classify/items?cid='+item.cid+'',index)">{{ item.title }}
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...mapMutations({
      SELECT_ITEM: "classify/SELECT_ITEM"
    }),
    ...
    this.getClassify({
      success: () => {
        this.$nextTick(() => {
          ...
          if (this.cid && this.classifys.length > 0) {
            let i = 0;
