  ...
  <div class="page">
    ...
    <div class='reviews-main'>
      ...
      <div v-show="reviews.length>0">
        ...
        <div class='reviews-more' @click="$router.replace('/goods/details/reviews?gid='+gid)">查看更多评价</div>
      </div>
      <div class="no-data" v-show="reviews.length<=0">暂无评价！</div>
    </div>
    <div class='bottom-btn-wrap'>
      <div class='btn fav' @click="joinFavs">收藏</div>
        ...
        <div class='attr-list' v-for="(item,index) in attrs" :key="index">
          ...
          <div class='val-wrap'>
            <span :class="{'val':true ,'active':item2.active}" v-for="(item2,index2) in item.values" :key="index2"
                  @click="SELECT_ATTRS({index:index,index2:index2})">{{ item2.value }}</span>
          </div>
        </div>
      </div>
...
export default {
  ...
  created() {
    ...
    this.getGoodsDetailsAttrs({gid: this.gid});

    this.getGoodsReviews({
      gid: this.gid, success: () => {
        this.$nextTick(() => {
          this.$utils.lazyImg();
        })
  ...
  computed: {
    ...mapState({
      details: state => state.classify.details,
      attrs: state => state.classify.attrs,
      reviews: state => state.reviews.reviews,
      total: state => state.reviews.total,
      isLogin: state => state.user.isLogin
    })
  },
  methods: {
    ...mapMutations({
      SELECT_ATTRS: "classify/SELECT_ATTRS",
      ADD_ITEM: "cart/ADD_ITEM",
    }),

    ...mapActions({
      getGoodsDetails: "classify/getGoodsDetails",
      getGoodsDetailsAttrs: "classify/getGoodsDetailsAttrs",
      getGoodsReviews: "reviews/getGoodsReviews",
      getGoodsReviewsPage: "reviews/getGoodsReviewsPage",
      addFavs: "user/addFavs"
    }),
    ...
    addCart() {
      if (this.isMove) {
        ...
        TweenMax.to(cloneImg, 2, {
          bezier: [{x: cloneImg.offsetLeft, y: -100}, {x: cartIcon.offsetLeft, y: -cartTop}], onComplete: () => {
            cloneImg.remove();
            this.isMove = true;
