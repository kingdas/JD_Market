    ...
    <div class='search-top'>
      ...
      <div class='search-header'>
        ...
        <div class='search-wrap' @click="searchShow.show=true">
          ...
          <div class='search-text'>{{ kwords }}</div>
        </div>
      ...
      <div>
        <div class='attr-wrap'>
          ...
          <div class='item-wrap' v-show="!isClassify">
            <div v-for="(item,index) in classifys" :key="index" :class="{item:true, active:item.active}"
                 @click="selectClassify({index:index})">{{ item.title }}
            </div>
          </div>
        </div>
          ...
          <div class='attr-title-wrap' @click="HIDE_PRICE()">
            ...
            <div class='price-wrap' @click.stop>
              <div class='price-input'><input type="tel" placeholder="最低价" :value="minPrice"
                                              @input="SET_MINPRICE({minPrice:$event.target.value})"/></div>
              <div class='price-line'></div>
              <div class='price-input'><input type="tel" placeholder="最高价" :value="maxPrice"
                                              @input="SET_MAXPRICE({maxPrice:$event.target.value})"/></div>
            </div>
            <div :class="{'attr-icon':true, up:priceData.isHide}"></div>
          </div>
          <div class='item-wrap' v-show="!priceData.isHide">
            <div :class="{item:true, active:item.active}" v-for="(item,index) in priceData.items" :key="index"
                 @click="SELECT_PRICE({index:index})">{{ item.minPrice }}-{{ item.maxPrice }}
            </div>
          </div>
        </div>
        ...
        <div>
          <div class='attr-wrap' v-for="(item,index) in attrs" :key="index">
            <div class='attr-title-wrap' @click="HIDE_ATTR({index:index})">
              ...
              <div :class="{'attr-icon':true, up:item.show}"></div>
            </div>
            <div class='item-wrap' v-show="item.show">
              <div :class="{item:true, active:item2.active}" v-for="(item2,index2) in item.param" :key="index2"
                   @click="SELECT_ATTR({index:index,index2:index2})">{{ item2.title }}
              </div>
            </div>
          </div>
          <div style='width:100%;height:1px;backgroundColor:#EFEFEF'></div>
...
export default {
  ...
  computed: {
    ...mapState({
      classifys: state => state.classify.classify,
      cid: state => state.goods.cid,
      priceData: state => state.goods.priceData,
      minPrice: state => state.goods.minPrice,
      maxPrice: state => state.goods.maxPrice,
      attrs: state => state.goods.attrs,
      params: state => state.goods.params,
      total: state => state.goods.total,
      searchData: state => state.goods.searchData,
    })
  },
  created() {
    this.otype = "all";
  ...
  methods: {
    ...mapMutations({
      HIDE_PRICE: "goods/HIDE_PRICE",
      SET_MINPRICE: "goods/SET_MINPRICE",
      SET_MAXPRICE: "goods/SET_MAXPRICE",
      SELECT_PRICE: "goods/SELECT_PRICE",
      HIDE_ATTR: "goods/HIDE_ATTR",
      SELECT_ATTR: "goods/SELECT_ATTR",
      SET_PARAM: "goods/SET_PARAM"
    }),

    ...mapActions({
      getClassify: "classify/getClassify",
      selectClassify: "goods/selectClassify",
      getAttrs: "goods/getAttrs",
      resetScreen: "goods/resetScreen",
      search: "goods/search",
      addSearch: "goods/addSearch",
    }),
    //禁用touchmove事件
      ...
      this.search({
        ...
        success: (pageNum) => {
          ...
          this.pullUp.init({"curPage": 1, "maxPage": parseInt(pageNum), "offsetBottom": 100}, (page) => {
            this.addSearch({...jsonParam, page: page});
          })
        }
      })
    },
    ...
    selectPriceOrder(index) {
      for (let i = 0; i < this.priceOrderList.length; i++) {
        if (this.priceOrderList[i].active === true) {
          this.priceOrderList[i].active = false;
          break;
        }
      }
      this.priceOrderList[index].active = true;
      this.$set(this.priceOrderList, index, this.priceOrderList[index]); //非动态改变数组中对象元素，不需要用Vue.set
  ...
  beforeRouteUpdate(to, from, next) {
    ...
    this.isPriceOrder = false;
    this.isSalesOrder = false;
    this.priceOrderList[0].active = true; //如果数组中的index为固定值，则不需要使用set方法
    this.otype = "all";
    this.resetScreen();
    this.init();
    this.getAttrs({
...
/*.goods-top .order-main .order-item .order-icon.up{background-image:url("../../../../assets/images/home/classify/up.png");background-size:100%;background-position: center;background-repeat: no-repeat;}*/
