  ...
  methods: {
      ...
      this.getOrder({
        status: this.status, page: 1, success: (pageNum) => {
          ...
          this.pullUp.init({"curPage": 1, "maxPage": parseInt(pageNum), "offsetBottom": 100}, (page) => {
            this.getOrderPage({status: this.status, page: page});
          })
        }
      })
    },
    ...
    cancelOrder(index, ordernum) {
      Dialog.confirm({
        ...
        this.asyncCancelOrder({index: index, ordernum: ordernum})
      }).catch(() => {
    ...
    sureOrder(index, item) {
      if (item.status !== "2") {
        this.asyncSureOrder({index:index,ordernum:item.ordernum,status:"2"});
      }
    }
  },
  created() {
