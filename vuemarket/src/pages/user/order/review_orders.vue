    ...
    <div class='order-list' v-for="(item,index) in reviewOrders" :key="index"
      ...
      <div class='item-list' v-for="(item2,index2) in item.goods" :key="index2">
        ...
        <div class='status-btn'
             @click.stop="$router.push('/user/order/addReviews?gid='+item2.gid+'&ordernum='+item.ordernum+'')">
...
export default {
  ...
  methods: {
    ...mapActions({
      getReviewOrders: "order/getReviewOrders",
      getReviewOrdersPage: "order/getReviewOrdersPage"
    }),
  },
  created() {
    ...
    this.getReviewOrders({
      page: 1, success: (pagenum) => {
        ...
        this.pullUp.init({"curPage": 1, "maxPage": pagenum, "offsetBottom": 100}, (page) => {
          this.getReviewOrdersPage({page: page});
        })
      }
    })
  },
