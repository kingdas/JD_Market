    ...
    <div class='main'>
      <div class='goods-list' v-for="(item,index) in favs" :key="index">
        ...
        <div class='btn-wrap'>
          ...
          <div class='btn' @click="delFavs(index,item.fid)">删除</div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...mapActions({
      ...
      getFavsPage: "user/getFavsPage",
      asyncDelFavs: "user/delFavs"
    }),
    ...
    delFavs(index, fid) {
      Dialog.confirm({
        ...
        this.asyncDelFavs({
          index: index, fid: fid, success: (res) => {
            if (res.code === 200) {
              this.$nextTick(() => {
                this.$utils.lazyImg();
              })
