  ...
  <div class="page">
    ...
    <div class='main'>
      <div class='list' v-for="(item,index) in address" :key="index"
           @click="$router.push('/user/address/mod?aid='+item.aid)">
        <div class='name-wrap'>
          <span>{{ item.name }}</span><span>{{ item.cellphone }}</span>
        </div>
        <div class='address'>
          <span
              v-if="item.isdefault==='1'">[默认]</span>{{ item.province }}{{ item.city }}{{ item.area }}{{ item.address }}
        </div>
...
export default {
  ...
  methods: {
    ...mapActions({
      getAddress: "address/getAddressList"
    })
  }
}
</script>
